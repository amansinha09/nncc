...
def get_score_fn_for_Task2(gold_standard_folder,system_output_folder):
    def calculate_score():
        corpora = Corpora(gold_standard_folder,system_output_folder)
        s_macro_f1 = []
        for tag in ['Disposition', 'NoDisposition', 'Undetermined']:
            evaluator_tag_s = MultipleEvaluator(corpora, tag)
            s_macro_f1.append(evaluator_tag_s.scores['tags']['micro']['f1'])
        return - statistics.mean(s_macro_f1)
    return calculate_score

def get_score_fn_for_Task3(gold_standard_folder,system_output_folder):
    def calculate_score():
        corpora = Corpora(gold_standard_folder,system_output_folder)
        evaluator_tag_s = MultipleEvaluator(corpora, 'Combined', mode='strict')
        return - evaluator_tag_s.scores['attributes']['micro']['f1']
    return calculate_score
